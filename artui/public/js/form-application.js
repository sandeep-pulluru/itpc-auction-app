...
functionByRecType['USER'] = 'iPostUser';
functionByRecType['ARTINV'] = 'iPostItem';
functionByRecType['AUCREQ'] = 'iPostAuctionRequest';
functionByRecType['BID'] = 'iPostBid';
functionByRecType['OPENAUC'] = 'iOpenAuctionForBids';
functionByRecType['XFER'] = 'iTransferItem';
...
function formApplication() {
    ...
    thisObj.submitForm = function(formButton) {
        ...
        */
        ...
        buttonhtml = $("#" + formButton.children("div")[0].id).html();
        ...
        if (formButton.children("div")[0] && formButton.children("div")[0].id == 'art_submit_auction') {
            ...
            args.push($.format.date(new Date(), "yyyyMMdd"));
            ...
            args.push("INIT");
            args.push($.format.date(new Date(), "yyyy-MM-dd hh:mm:ss"));
            args.push($.format.date(new Date(new Date().getTime() + 30 * 60000), "yyyy-MM-dd hh:mm:ss"));
            args.push($.format.date(new Date(), "yyyy-MM-dd hh:mm:ss"));
            console.log(args);
            ...
            myArgs.push($.format.date(new Date(), "yyyy-MM-dd HH:mm:ss"));
            var method = "invoke";
        ...
        if (recType == "USER") {
            var method = "query";
            payload = constructPayload(method, "qGetUser", args);
            makeRestCall(payload, method, recType, args);
            return;
        } else if (recType == "ARTINV") {
            var method = "query";
            payload = constructPayload(method, "qGetItem", args);
            //makeRestCall(payload, method, recType, args);
            globalItemData = args;
            $.ajax({
                url: mainApp.URL,
                ...
                success: function(data, textStatus, jqXHR) {
                    if (data["error"] && data["error"].message && method == "query") {
                        ...
                        newPayload = constructPayload("invoke", "iPostItem", globalItemData);
    ...
    thisObj.getItemTransfer = function() {
        ...
        args.push("2016")
        var method = "query"
        payload = constructPayload(method, "qGetItemListByCat", args);
    ...
    thisObj.populateItemDetail = function(data) {
        ...
        args.push("VERIFY")
        var payload = constructPayload(method, "qIsItemOnAuction", args);
...
function makeRestCall(payload, method, recordType, argsData, funcName) {
    ...
    if (typeof clickedbutton != 'undefined' && clickedbutton != '')
    {
        console.log(clickedbutton);
        $("#" + clickedbutton.children("div")[0].id).html("Processing...");
    }
    ...
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR) {
            if (typeof clickedbutton != 'undefined' && clickedbutton != '')
                $("#" + clickedbutton.children("div")[0].id).html(buttonhtml)
            ...
            if (data["error"] && data["error"].message) {
                if (method == "query") {
                    console.log("Query is failed !! <br/><b>Error:</b> " + data["error"].message)
                    //Resend the data again
                    globalData.push($.format.date(new Date(), "yyyy-MM-dd HH:mm:ss"));
                    newPayload = constructPayload("invoke", "iPostUser", globalData);
...
checkForNotification = function(itemId) {
    ...
    setTimeout(function() {
        var method = 'query'
        var payload = constructPayload(method, "qGetItem", args);
...
checkIfAuctionClosed = function(auctionID) {
    ...
    setTimeout(function() {
        ...
        payload = constructPayload(method, "qGetAuctionRequest", newArgs);
