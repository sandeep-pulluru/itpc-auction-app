function tableApplication(){
	...
	thisObj.CloseAuctionsPoll = function(){
		var method = "invoke";
		var args = ["2016", "CLAUC", $.format.date(new Date(), "yyyy-MM-dd HH:mm:ss")];
		var functionName = "iCloseOpenAuctions";
	...
	thisObj.getTableAuction = function(auctionID){
		...
		thisObj.populateTableDetails(auctionID, "qGetAuctionRequest");
	...
	thisObj.getTableItem = function(itemID){
		...
		//RETURN API DATA TO "populateTableDetails" FUNCTION BELOW.
		//REMOVE DEBUG LINE BELOW
		thisObj.populateTableDetails(itemID, "qGetItem");
  ...
  //TODO: Remove if not used
	thisObj.getTableOwner = function(ownerID){
		...
		//RETURN API DATA TO "populateTableDetails" FUNCTION BELOW.
		//REMOVE DEBUG LINE BELOW
		thisObj.populateTableDetails(ownerID, "qGetUser");
	...
	thisObj.getAuctions = function(){
		...
		var functionName = "qGetListOfInitAucs";
	...
	thisObj.UpdateAuctionTimers = function(){
		...
		$('.table-cell[name="timer-display"]').each(function(index){
			...
			var now = new Date();
			var currentUtcTime = new Date().getTime();//-875000;
			var momentDate = moment(endTime, 'YYYY-MM-DD HH:mm:ss');
			var endDate = momentDate.toDate();			
			var endTimeDiff = endDate.getTime()-now.getTime();
	...
	thisObj.getBidding = function(){
		...
		//REMOVE DEBUG LINE BELOW
		var functionName = "qGetListOfOpenAucs";
		var payload = constructPayload("query",functionName , ["2016"])
		RestCall(payload, "query", functionName);
	...
	thisObj.populateBidding = function(data){
		...
		for (var property in obj) {
			//thisObj.populateTableRow('1111','001','200',0);
			//var endDate = new Date(new Date(obj[count].CloseDate)).getTime();
			thisObj.populateBiddingRow(obj[count].AuctionID, obj[count].ItemID, obj[count].AuctionHouseID, obj[count].CloseDate, count, obj[count].ReservePrice, obj[count].BuyItNowPrice);
			count++;
		}
		thisObj.setTableEvents();
    thisObj.setDetailEvents();
    ...
    //TODO: Split the value to AuctionID+ ItemID + Current Ownerra ID : 7967-1000-200
		...
		var updateBids = setInterval(function(){
			...
			if (thisObj.auctionID ){
				...
				//args.push (key);
				/*var functionName = "qGetLastBid";
				payload = constructPayload(method, functionName, args);
				RestCall(payload, method, functionName);*/
				functionName = "qGetHighestBid";
				payload = constructPayload(method, functionName, args);
				RestCall(payload, method, functionName);
			}
...
RestCall = function (payload, method, functionName, auctionID){
	...
	$.ajax({
		...
		if (data["error"] && data["error"].message) {
			...
			if (method == "query"){
 				switch (functionName){
					case "qGetAuctionRequest":
					case "qGetItem":
					case "qGetUser":
						tableApp.populateSubTable(res);
						break;
					case "qGetListOfInitAucs":
						tableApp.populateAuctionsTable(res);
						break;
					case "qGetListOfOpenAucs":
					  console.log(res);
